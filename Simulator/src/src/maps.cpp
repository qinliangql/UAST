...
Maps::setParam(const YAML::Node& config)
{
  ...
  // hall
  num_pillars = config["num_pillars"].as<int>();                      // 柱子数量
  num_short_walls = config["num_short_walls"].as<int>();                  // 短墙数量
...
void Maps::hall()
{
  double _resolution = 1 / info.scale;
  // 大厅尺寸设置
  double hall_L = info.sizeX / info.scale;  // 大厅长度
  double hall_W = info.sizeY / info.scale;  // 大厅宽度
  double hall_H = info.sizeZ / info.scale;  // 大厅高度
  double wall_W = 0.3;                      // 围墙宽度
  double pillar_W = 0.4;                    // 柱子宽度
  double short_wall_L_min = 3.0;            // 短墙最小长度
  double short_wall_L_max = 5.0;            // 短墙最大长度
  ...
  // 创建随机引擎
  std::default_random_engine eng(info.seed);
  std::uniform_real_distribution<double> rand_pos(0, 1.0);
  std::uniform_real_distribution<double> rand_rot(0, 2 * M_PI);
  std::uniform_real_distribution<double> rand_short_wall_L(short_wall_L_min, short_wall_L_max);
  ...
  // 生成四周围墙
  // 左围墙
  pcl::PointCloud<pcl::PointXYZ>::Ptr left_wall(new pcl::PointCloud<pcl::PointXYZ>);
  generateWallWithWindows(left_wall, hall_L, wall_W, hall_H, 0);  // 0个窗户
  translation = Eigen::Vector3f(-hall_L /2, -hall_W /2, 0);
  pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_left_wall(new pcl::PointCloud<pcl::PointXYZ>);
  transformPointCloud(left_wall, transformed_left_wall, rotation0, translation);
  *info.cloud += *transformed_left_wall;

  // 右围墙
  pcl::PointCloud<pcl::PointXYZ>::Ptr right_wall(new pcl::PointCloud<pcl::PointXYZ>);
  generateWallWithWindows(right_wall, hall_L, wall_W, hall_H, 0);
  translation = Eigen::Vector3f(-hall_L /2, hall_W /2, 0);
  pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_right_wall(new pcl::PointCloud<pcl::PointXYZ>);
  transformPointCloud(right_wall, transformed_right_wall, rotation0, translation);
  *info.cloud += *transformed_right_wall;

  // 前围墙
  pcl::PointCloud<pcl::PointXYZ>::Ptr front_wall(new pcl::PointCloud<pcl::PointXYZ>);
  generateWallWithWindows(front_wall, hall_L, wall_W, hall_H, 0);
  translation = Eigen::Vector3f(hall_L /2, -hall_W /2, 0);
  pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_front_wall(new pcl::PointCloud<pcl::PointXYZ>);
  transformPointCloud(front_wall, transformed_front_wall, rotation90, translation);
  *info.cloud += *transformed_front_wall;

  // 后围墙
  pcl::PointCloud<pcl::PointXYZ>::Ptr back_wall(new pcl::PointCloud<pcl::PointXYZ>);
  generateWallWithWindows(back_wall, hall_L, wall_W, hall_H, 0);
  translation = Eigen::Vector3f(-hall_L /2, -hall_W /2, 0);
  pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_back_wall(new pcl::PointCloud<pcl::PointXYZ>);
  transformPointCloud(back_wall, transformed_back_wall, rotation90, translation);
  *info.cloud += *transformed_back_wall;

  // 生成中间的柱子
  for (int i = 0; i < num_pillars; ++i)
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr pillar(new pcl::PointCloud<pcl::PointXYZ>);
    
    // 生成柱子点云
    for (float x = -pillar_W / 2; x <= pillar_W / 2; x += 0.1f)
    {
      for (float y = -pillar_W / 2; y <= pillar_W / 2; y += 0.1f)
      {
        for (float z = 0; z <= hall_H; z += 0.1f)
        {
          pillar->points.emplace_back(x, y, z);
        }
      }
    }
    
    // 随机位置放置柱子，但避开墙壁
    double pillar_x = (rand_pos(eng) - 0.5) * hall_L * 0.8;  // 0.8系数确保柱子在大厅内部
    double pillar_y = (rand_pos(eng) - 0.5) * hall_W * 0.8;
    
    translation = Eigen::Vector3f(pillar_x, pillar_y, 0);
    pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_pillar(new pcl::PointCloud<pcl::PointXYZ>);
    transformPointCloud(pillar, transformed_pillar, rotation0, translation);
    *info.cloud += *transformed_pillar;
  }

  // 生成中间的短墙
  for (int i = 0; i < num_short_walls; ++i)
  {
    double short_wall_L = rand_short_wall_L(eng);
    pcl::PointCloud<pcl::PointXYZ>::Ptr short_wall(new pcl::PointCloud<pcl::PointXYZ>);
    generateWallWithWindows(short_wall, short_wall_L, wall_W, hall_H, 0);  // 0个窗户  
    
    // 随机位置和旋转角度放置短墙
    double wall_x = (rand_pos(eng) - 0.5) * hall_L * 0.65;
    double wall_y = (rand_pos(eng) - 0.5) * hall_W * 0.65;
    ...
    translation = Eigen::Vector3f(wall_x, wall_y, 0);
    pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_short_wall(new pcl::PointCloud<pcl::PointXYZ>);
    transformPointCloud(short_wall, transformed_short_wall, rotation, translation);

    *info.cloud += *transformed_short_wall;
  }
  ...
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr ground_cloud = generateGround(info.cloud, _resolution);
    pcl::PointCloud<pcl::PointXYZ>::Ptr ceiling_cloud = generateGround(info.cloud, _resolution, hall_H - _resolution);
    *info.cloud += *ground_cloud;
    *info.cloud += *ceiling_cloud;
  }
  else
