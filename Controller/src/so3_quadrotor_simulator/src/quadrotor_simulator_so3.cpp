...
main(int argc, char** argv)
{
  ...
  std::string quad_name;
  n.param("quadrotor_name", quad_name, std::string("odom"));
  ...
  */
  ...
  while (n.ok())
  {
    ...
    if (tnow >= next_odom_pub_time)
    {
      ...
      odomToTF(odom_msg, transformStamped, quad_name);
...
void 
odomToTF(const nav_msgs::Odometry& odom_msg, geometry_msgs::TransformStamped& transformStamped, std::string& quad_name) {
